...
- 2026-06-08 — retired `packages_universe` table. Signals (`downloads_last_30d`, `centrality_score`, `rank_in_ecosystem`) migrated onto `packages`. `rank_packages_universe()` rewritten to operate directly on `packages` scoped by `critical_top_n_by_ecosystem` JSONB keys. §Downloads timeline by tier simplified (Tier 2/3 split removed). §Write semantics `packages_universe` row removed.
---
