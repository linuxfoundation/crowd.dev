...
| `repos`                               | Registry workers (npm, Maven) do **not** write `repos` metadata. They may INSERT a bare `repos(url)` row (canonical url only, all other columns NULL) solely to create the FK target their `package_repos` link needs — `host`/`owner`/`name` and all metadata stay NULL and remain enricher-owned. The GitHub enricher — triggered when `repos.last_synced_at IS NULL` — upserts `repos` with metadata. Docker Hub worker adds `docker_*` columns on top. |
