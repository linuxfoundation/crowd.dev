...
We use the [deps.dev BigQuery public datasets](https://deps.dev) — specifically `PackageVersionsLatest`, `DependentsLatest`, `PackageVersionToProjectLatest`, and `ProjectsLatest` — filtered to `System IN ('NPM', 'MAVEN')` as the universe input. The BigQuery data is exported to Parquet files and imported directly into the `packages` table on a weekly cadence aligned with deps.dev's own refresh interval. The first run is a one-time full backfill; subsequent weekly imports only pull rows whose deps.dev snapshot date has advanced since the previous import, so the export size and write volume are scoped to actual diffs rather than the full universe. A scoring + ranking job then promotes the top-N per ecosystem by setting `is_critical = true` directly on `packages`.
...
The §Universe source section above establishes that `packages` is the single source of truth for both reading and writing, and that `rank_packages()` produces the criticality scores. This section locks in **what goes into the score** — replacing the placeholder formula (`X * downloadsCount + Y * dependentCount`) with a defensible methodology that captures load-bearing upstream packages (the left-pad / XZ pattern), normalizes across ecosystems, and supports manual overrides for known-critical primitives.
...
| `downloads_last_30d`                  | Upsert on `(purl, end_date)`. Written by the npm worker for `is_critical = TRUE` packages. The cached `packages.downloads_last_30d` column must be updated in the same pass.                                                                                         |
...
| Table             | Scope      | Grain                       | Unique key           | PK           | Partitioning                   |
| ----------------- | ---------- | --------------------------- | -------------------- | ------------ | ------------------------------ |
| `downloads_daily` | `packages` | one row per package per day | `(package_id, date)` | `(id, date)` | RANGE on `date` via pg_partman |

   **Rolling-30-day window shape**: each window uses `end_date = 1st of calendar month, start_date = end_date − 30 days` — not a true calendar month. This ensures every window covers exactly 30 days, making download counts directly comparable across months for criticality scoring. Calendar months (28–31 days) would skew comparisons.
...
- 2026-06-08 — retired `packages_universe` as the ranking workspace; `packages` is now the single source of truth for all reading and writing. `rank_packages_universe()` replaced by `rank_packages()` which reads and writes `packages` directly. Download refresh scoped to `is_critical = TRUE` packages; `packages.downloads_last_30d` is the single downloads column used by `rank_packages()`.
---
