...
exports.getAkritesExternalContactDetailBatch = getAkritesExternalContactDetailBatch;
...
async function getAkritesExternalContactDetailBatch(req, res) {
    ...
    const rows = await (0, data_access_layer_1.getContactDetailsByPurls)(qx, normalizedPurls);
    ...
    const results = pagedPurls.map((requestedPurl, i) => {
        ...
        return {
            requestedPurl,
            found: row !== undefined,
            contact: row ? (0, akritesExternalContactDetail_1.toAkritesExternalContactDetail)(row) : null,
        };
    });
    (0, api_1.ok)(res, { page, pageSize, total, results });
}
//# sourceMappingURL=getAkritesExternalContactDetailBatch.js.map