...
function main() {
  ...
  console.error(
    `Parsed ${countries.length - 1} countries from country_mapping.pipe, plus the 'XX' unknown-location sentinel row`,
  )
  ...
  const rows = countries.map(({ country, flag, country_code, timezone_offset }) => {
    ...
    if (match) {
      ...
      console.error(
        `WARNING: no mledoze match for country_code=${country_code} (${country}) — defaulting region/subregion to "${UNKNOWN}"`,
      )
    }
  ...
  fs.writeFileSync(
    path.join(FIXTURES_DIR, 'country_mapping_ds.ndjson'),
    withFlags.join('\n') + '\n',
  )
  ...
  fs.writeFileSync(
    path.join(FIXTURES_DIR, 'country_mapping_no_flags_ds.ndjson'),
    noFlags.join('\n') + '\n',
  )

  console.error(
    `Wrote ${rows.length} rows to country_mapping_ds.ndjson and country_mapping_no_flags_ds.ndjson`,
  )
}
