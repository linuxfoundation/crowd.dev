...
Push Tinybird datasource and pipe files using "tb push".
Datasources are always pushed first to ensure dependencies are met.
...
EXAMPLES:
  push.sh
    Push all datasource and pipe files
...
for file in "$DATASOURCES_FOLDER"/*; do
  if [ -f "$file" ]; then
    ...
    tb push "$DATASOURCES_FOLDER/$local_basename" --force --yes --no-check
  fi
done
...
for file in "$PIPES_FOLDER"/*; do
  if [ -f "$file" ]; then
    ...
    tb push "$PIPES_FOLDER/$local_basename" --force --yes --no-check
  fi
done