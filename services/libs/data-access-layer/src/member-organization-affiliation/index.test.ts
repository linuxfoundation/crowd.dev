...
describe('deleteMemberOrganizations MSA cleanup', () => {
  async function findActiveMsas(qx: QueryExecutor, memberId: string, organizationId: string) {
    return qx.select(
  ...
  test('default path removes orphan MSA when last MO is deleted', async ({ qx }) => {
    const [org] = await createOrganizations(qx, withOrganizationDefaults([{ displayName: 'Acme' }]))
    const [member] = await createMembers(qx, withMemberDefaults([{}]))
    const { kubernetes } = await createLeafSegments(qx)
    ...
    expect(await findActiveMsas(qx, member.id, org.id)).toHaveLength(1)

    await deleteMemberOrganizations(qx, member.id, { ids: [mo.id] })

    expect(await findActiveMsas(qx, member.id, org.id)).toHaveLength(0)
  })

  test('skipMsaCleanup preserves MSA when last MO is deleted', async ({ qx }) => {
    const [org] = await createOrganizations(qx, withOrganizationDefaults([{ displayName: 'Acme' }]))
    const [member] = await createMembers(qx, withMemberDefaults([{}]))
    const { kubernetes } = await createLeafSegments(qx)
    ...
    await deleteMemberOrganizations(qx, member.id, { ids: [mo.id], skipMsaCleanup: true })
