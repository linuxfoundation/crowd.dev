...
describe.skipIf(!HAVE_DB)('packagist transitive dependents — real packages-db', () => {
  ...
  describe('closure + merge — fixture-only edge set', () => {
    ...
    it('refuses to merge when the counts staging table is empty', async () => {
      ...
      await expect(mergePackagistTransitiveCounts(qx, minFixtureId(), 3)).rejects.toThrow(/empty/i)
    })
  })
  ...
  describe('packagist_transitive_runs ledger', () => {
    it('walks the run lifecycle: pending (reusable) → merging with graph sizes → done with totals', async () => {
      ...
      // newest pending wins — a Temporal retry reuses it instead of minting a third
      expect(await findPendingPackagistTransitiveRun(qx)).toBe(runB)

      await markPackagistTransitiveRunMerging(qx, runB, {
        edgeCount: 918346,
        packagesWithDependents: 85600,
      })
      expect(await findPendingPackagistTransitiveRun(qx)).toBe(runA)
