...
```
BQ_DATASET_INGEST_<KIND>_MAX_BQ_GB=<number>
```

where `<KIND>` is the job kind in `UPPER_SNAKE_CASE`.

**When adding a new job kind, add a row to this table.**

| Env var override                                       | Default (GB) | Job kind              | Notes                                                     |
| ------------------------------------------------------ | -----------: | --------------------- | --------------------------------------------------------- |
| `BQ_DATASET_INGEST_PACKAGES_MAX_BQ_GB`                 | 6000 / 400   | `packages`            | 6000 full, 400 incremental (set in `ingestPackages.ts`)   |
