...
| `BQ_DATASET_INGEST_DEPENDENT_COUNTS_RUBYGEMS_MAX_BQ_GB` | 200 | `dependent_counts_rubygems` | RUBYGEMS exact reverse transitive closure over `RubyGemsRequirementsLatest` (script mode, `RuntimeDependencies` only, matching `package_dependencies`). All 3 count columns. Corpus is NUGET-scale (~1.7M pkgs / ~4.5M edges, verified via BQ 2026-07-13); `maximumBytesBilled` runaway cap set accordingly.                                     |
