...
export async function listParquetFiles(
  input: ListParquetFilesInput,
): Promise<ListParquetFilesOutput> {
  ...
  log.info(
    { gcsPrefix: input.gcsPrefix, fileCount: fileNames.length, totalBytes, totalRows },
    'Listed parquet files',
  )
