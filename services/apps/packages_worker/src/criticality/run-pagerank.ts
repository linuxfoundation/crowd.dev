...
async function main() {
  ...
  edges.length = 0
  console.log(`  ${graph.N.toLocaleString()} nodes  ${edgeCount.toLocaleString()} edges  (${((Date.now()-t)/1000).toFixed(1)}s)\n`)
  ...
  console.log(`  nodes=${result.nodeCount.toLocaleString()}  edges=${result.edgeCount.toLocaleString()}  iters=${result.iterations}  duration=${(result.durationMs/1000).toFixed(1)}s`)
