...
// Shared 429-retry/backoff GET used by both @latest and @v/list — kept close to the
// existing fetchLatest scaffolding rather than abstracted, since the two callers
// differ only in URL suffix and response parsing.
...
// GOPROXY @v/list: plain text, one version per line, no ordering guarantee.
export async function fetchVersionList(
  module: string,
  timeoutMs: number,
): Promise<string[] | FetchError> {
  const url = `${BASE}/${escapeModulePath(module)}/@v/list`
  ...
  try {
    ...
    return { kind: 'MALFORMED', message: 'failed to read @v/list body' }
  }
