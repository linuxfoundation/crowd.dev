...
import { scanNuGetDependents } from './dependentsScanNuGet'

export async function runDependentsStageNuGet(
  qx: QueryExecutor,
  analysisId: string,
  onProgress?: () => void,
  signal?: AbortSignal,
  ...
  try {
    ...
    const scanResult = await scanNuGetDependents(
      qx,
    ...
    const packageIdsByName = await findPackageIdsByName(qx, 'nuget', names)
